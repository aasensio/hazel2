...
!*********************************  
    ...
    if (params(index)%recompute_see_rtcoef) then
        ...
        call fill_SEE(params(index), fixed(index), 1, .FALSE.)
        ...
        call calc_rt_coef(params(index), fixed(index), observation(index), 1, .FALSE.)
...
subroutine c_hazel_rt(index, B1Input, hInput, logn1Input, boundaryInput, &
    transInput, anglesInput, nLambdaInput, lambdaAxisInput, dopplerWidthInput, dampingInput, &
    dopplerVelocityInput, betaInput, nbarInput, omegaInput, nSlabsInput, dz, nquad_photosphere, nquad_nophotosphere, &
    wavelengthOutput, stokesOutput, epsOutput, etaOutput, rhoOutput, error) bind(c)

    integer(c_int), intent(in) :: transInput, index
    integer(c_int), intent(in) :: nLambdaInput, nSlabsInput, nquad_photosphere, nquad_nophotosphere
    real(c_double), intent(in), dimension(nLambdaInput) :: lambdaAxisInput
    real(c_double), intent(in), dimension(3) :: B1Input, anglesInput
    real(c_double), intent(in), dimension(4,nLambdaInput) :: boundaryInput
    real(c_double), intent(in), dimension(4) :: nbarInput, omegaInput
    real(c_double), intent(in) :: hInput, logn1Input, dopplerWidthInput, dampingInput, dopplerVelocityInput, betaInput, dz
    real(c_double), intent(out), dimension(nLambdaInput) :: wavelengthOutput
    real(c_double), intent(out), dimension(4,nLambdaInput) :: stokesOutput, epsOutput, etaOutput
    real(c_double), intent(out), dimension(3,nLambdaInput) :: rhoOutput
    integer(c_int), intent(out) :: error
...
! Set the variables for the experiment from the parameters of the subroutine
    ...
    idep = 0
    use_paschen_back = 1
    params(index)%nslabs = nSlabsInput
    
    params(index)%bgauss = B1Input(1)
    params(index)%thetabd = B1Input(2)
    params(index)%chibd = B1Input(3)
    params(index)%height = hInput
    params(index)%logn = logn1Input
    params(index)%beta = betaInput
    ...
    fixed(index)%chid = anglesInput(2)
    fixed(index)%gammad = anglesInput(3)
    fixed(index)%nslabs = nslabsInput
    fixed(index)%nmus_photo = nquad_photosphere
    fixed(index)%nmus_nophoto = nquad_nophotosphere
...
!*********************************  
    ...
    if (.not.associated(inversion(index)%stokes_unperturbed)) allocate(inversion(index)%stokes_unperturbed(0:3,fixed(index)%no))
    if (.not.associated(fixed(index)%stokes_boundary)) allocate(fixed(index)%stokes_boundary(0:3,observation(index)%n))
    if (.not.associated(observation(index)%freq)) allocate(observation(index)%freq(observation(index)%n))
    
    fixed(index)%stokes_boundary(0:3,:) = boundaryInput

    observation(index)%wl = lambdaAxisInput
    observation(index)%freq = -PC / (fixed(index)%wl*1.d-8)**2 * (lambdaAxisInput*1.d-8)
    
    fixed(index)%omax = minval(lambdaAxisInput)
    fixed(index)%omin = maxval(lambdaAxisInput)

! Do the synthesis
    call do_transfer(params(index), fixed(index), observation(index), inversion(index)%stokes_unperturbed, error)
