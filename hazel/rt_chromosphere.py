...
class Hazel_atmosphereRT(General_atmosphere):
    ...
    def print_parameters(self, first=False, error=False):
        if (self.coordinates_B == 'cartesian'):
            self.logger.info("     {0}        {1}        {2}        {3}       {4}       {5}      {6}      {7}     {8}".format('Bx', 'By', 'Bz', 'logn', 'v', 'deltav', 'beta', 'a', 'ff'))
            self.logger.info("{0:8.3f}  {1:8.3f}  {2:8.3f}  {3:8.3f}  {4:8.3f}  {5:8.3f}  {6:8.3f}  {7:8.3f}   {8:8.3f}".format(self.parameters['Bx'], self.parameters['By'], self.parameters['Bz'], self.parameters['logn'], 
                self.parameters['v'], self.parameters['deltav'], self.parameters['beta'], self.parameters['a'], self.parameters['ff']))
            
            if (error):            
                self.logger.info("{0:8.3f}  {1:8.3f}  {2:8.3f}  {3:8.3f}  {4:8.3f}  {5:8.3f}  {6:8.3f}  {7:8.3f}".format(self.error['Bx'], self.error['By'], self.error['Bz'], self.error['logn'], 
                self.error['v'], self.error['deltav'], self.error['beta'], self.error['a']))
        
        if (self.coordinates_B == 'spherical'):
            self.logger.info("     {0}        {1}        {2}        {3}       {4}       {5}      {6}      {7}".format('B', 'thB', 'phiB', 'logn', 'v', 'deltav', 'beta', 'a'))
            self.logger.info("{0:8.3f}  {1:8.3f}  {2:8.3f}  {3:8.3f}  {4:8.3f}  {5:8.3f}  {6:8.3f}  {7:8.3f}".format(self.parameters['B'], self.parameters['thB'], self.parameters['phiB'], self.parameters['logn'], 
                self.parameters['v'], self.parameters['deltav'], self.parameters['beta'], self.parameters['a']))
            
            if (error):            
                self.logger.info("{0:8.3f}  {1:8.3f}  {2:8.3f}  {3:8.3f}  {4:8.3f}  {5:8.3f}  {6:8.3f}  {7:8.3f}".format(self.error['B'], self.error['thB'], self.error['phiB'], self.error['logn'], 
                self.error['v'], self.error['deltav'], self.error['beta'], self.error['a']))
    ...
    def synthesize(self, stokes=None, returnRF=False, nlte=None):
        ...
        nSlabsInput = self.n_slabs
        
        args = (self.index, B1Input, hInput, n1Input, boundaryInput, transInput, 
            anglesInput, nLambdaInput, lambdaAxisInput, dopplerWidthInput, 
            dampingInput, dopplerVelocityInput, 
            betaInput, nbarInput, omegaInput, nSlabsInput, self.dz, self.nquad_photosphere, self.nquad_nophotosphere)
        
        l, stokes, self.eps, self.eta, self.rho, error = hazel_code._synthRT(*args)
